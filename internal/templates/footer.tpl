<!-- Alerts -->
<div class="container">
    <div id="footer_alert" class="alert alert-info" style="display:none; z-index: 100">
        <a href="#" class="close" aria-label="close">&times;</a>
        <span id="footer_alert_value"></span>
    </div>
</div>
<!-- Alerts end -->
<footer class="footer">
    <div class="container">
        <div class="col-md-4">
            <div class="navbar-text">
                <p>
                    Created &amp; Maintained by <strong><a href="https://inventivetalent.org/?utm_source=spiget&utm_medium=author_link">inventivetalent</a></strong>
                </p>
            </div>
        </div>
        <div class="col-md-4">
            <div class="navbar-text">
                <p>
                    <a href="https://legal.inventivetalent.org/terms">Terms of Service</a> | <a href="https://legal.inventivetalent.org/privacy">Privacy Policy</a> | <a href="https://legal.inventivetalent.org/imprint">Imprint</a>
                </p>
            </div>
        </div>
        <div class="col-md-4">
            <div class="navbar-text">
                <a href="https://twitter.com/Spiget_org" class="twitter-follow-button" data-show-count="false">Follow @Spiget</a>
                <a href="https://twitter.com/Inventivtalent" class="twitter-follow-button" data-show-count="false">Follow @inventivetalent</a>

                &nbsp;

                <a href="https://github.com/SpiGetOrg" target="_blank"><img src="/img/GitHub-Mark-32px.png" width="32" height="32" alt="GitHub"></a>
            </div>
        </div>
    </div>
</footer>
